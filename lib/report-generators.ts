...
// Replace the generatePDF function with this improved version that uses jsPDF

// Then replace the generatePDF function with this implementation:
export async function generatePDF(data: ReportData): Promise<void> {
  ...
  const skillTableData = data.skillScores.map((item) => [item.skill, `${item.score}/10`])
